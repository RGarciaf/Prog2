...
Statuslist_insertFirst(List *list, const void *pelem); 

/* Inserta al final de la lista realizando una copia de la información recibida. */
Statuslist_insertLast(List *list, const void *pelem);   

/* Inserta en orden en la lista realizando una copia del elemento. */
Statuslist_insertInOrder(List *list, const void *pelem);

/* Extrae del principio de la lista, devolviendo directamente el puntero al campo info del nodo extraído, nodo que finalmentees liberado. OJO: tras guardar la dirección del campo info que se va a devolver y antes de liberar el nodo, pone el campo info del nodo a NULL, para que no siga apuntando a la info a devolver y, por tanto, no la libere al liberar el nodo */
void *list_extractFirst(List *list); 

/* Extrae del final de la lista, devolviendo directamente el puntero al campo info del nodo extraído, nodo que finalmente es liberado. OJO: tras guardar la dirección del campo info que se va a devolver y antes de liberar el nodo, pone el campo info del nodo a NULL, para que no siga apuntando a la info a devolver y, por tanto, no la libere al liberar el nodo */
...
int list_size(const List *list);                                                                                                                                                                                                                                                                    /* Imprime una lista devolviendo el número de caracteres escritos. */
